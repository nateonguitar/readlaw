...
function edShowLinks() {
	var tempStr = '<select onchange="edQuickLink(this.options[this.selectedIndex].value, this);"><option value="-1" selected>' + multiquicktagsL10n.quickLinks + '</option>', i;
...
function edSpell(myField) {
	...
	if (word == '') {
		word = prompt(multiquicktagsL10n.wordLookup, '');
	}
...
function edToolbar() {
	...
	document.write('<input type="button" id="ed_spell" class="ed_button" onclick="edSpell(edCanvas);" title="' + multiquicktagsL10n.dictionaryLookup + '" value="' + multiquicktagsL10n.lookup + '" />');
	document.write('<input type="button" id="ed_close" class="ed_button" onclick="edCloseAllTags();" title="' + multiquicktagsL10n.closeAllOpenTags + '" value="' + multiquicktagsL10n.closeTags + '" />');
...
function edInsertLink(myField, i, defaultValue) {
	...
	if (!edCheckOpenTags(i)) {
		var URL = prompt(multiquicktagsL10n.enterURL, defaultValue);
		if (URL) {
...
function edInsertImage(myField) {
	var myValue = prompt(multiquicktagsL10n.enterImageURL, 'http://');
	if (myValue) {
		myValue = '<img src="'
				+ myValue
				+ '" alt="' + prompt(multiquicktagsL10n.enterImageDescription, '')
				+ '" />';
...
var QTags = function(name, id, container, disabled) {
	...
	t.edShowButton = function(button, i) {
		if ( disabled && (disabled.indexOf(','+button.display+',') != -1) )
			return '';
		else if ( button.id == button_prefix+'_img' )
		...
		else if (button.id == button_prefix+'_link')
	...
	t.Buttons[t.Buttons.length] = new edButton(button_prefix+'_strong','b','<strong>','</strong>','b');
	t.Buttons[t.Buttons.length] = new edButton(button_prefix+'_em','i','<em>','</em>','i');
	t.Buttons[t.Buttons.length] = new edButton(button_prefix+'_link','link','','</a>','a'); // special case
	t.Buttons[t.Buttons.length] = new edButton(button_prefix+'_block','b-quote','\n\n<blockquote>','</blockquote>\n\n','q');
	t.Buttons[t.Buttons.length] = new edButton(button_prefix+'_del','del','<del datetime="' + datetime + '">','</del>','d');
	t.Buttons[t.Buttons.length] = new edButton(button_prefix+'_ins','ins','<ins datetime="' + datetime + '">','</ins>','s');
	t.Buttons[t.Buttons.length] = new edButton(button_prefix+'_img','img','','','m',-1); // special case
	t.Buttons[t.Buttons.length] = new edButton(button_prefix+'_ul','ul','<ul>\n','</ul>\n\n','u');
	t.Buttons[t.Buttons.length] = new edButton(button_prefix+'_ol','ol','<ol>\n','</ol>\n\n','o');
	t.Buttons[t.Buttons.length] = new edButton(button_prefix+'_li','li','\t<li>','</li>\n','l');
	t.Buttons[t.Buttons.length] = new edButton(button_prefix+'_code','code','<code>','</code>','c');
	t.Buttons[t.Buttons.length] = new edButton(button_prefix+'_more','more','<!--more-->','','t',-1);
//	t.Buttons[t.Buttons.length] = new edButton(button_prefix+'_next','page','<!--nextpage-->','','p',-1);

	tb = document.createElement('div');
	tb.id = name+'_qtags';

	html = '<div id="'+name+'_toolbar" class="ed_toolbar">';
	for (i = 0; i < t.Buttons.length; i++)
		html += t.edShowButton(t.Buttons[i], i);

	html += '<input type="button" id="'+name+'_ed_spell" class="ed_button" onclick="edSpell('+name+'.Canvas);" title="' + multiquicktagsL10n.dictionaryLookup + '" value="' + multiquicktagsL10n.lookup + '" />';
	html += '<input type="button" id="'+name+'_ed_close" class="ed_button" onclick="'+name+'.edCloseAllTags();" title="' + multiquicktagsL10n.closeAllOpenTags + '" value="' + multiquicktagsL10n.closeTags + '" /></div>';

...
<?php
    ...
    else
    {
        ...
        else
        {
            ...
            if($object->ui['reorder']!==null&&$object->ui['action']=='reorder')
            {
                $object->findRecords($object->ui['reorder_sort'],pods_sanitize($object->ui['reorder_limit']),$object->ui['reorder_where'],$object->ui['reorder_sql']);
            }
            else
            {
                $object->findRecords($object->ui['sort'],$object->ui['limit'],$object->ui['where'],$object->ui['sql']);
            }
...
?>
        <div class="alignleft actions"><?php if(!in_array('add',$object->ui['disable_actions'])) { ?>
            <input type="button" value="Add New <?php echo $object->ui['item']; ?>" class="button" onclick="document.location='<?php echo pods_ui_var_update(array('action'.$object->ui['num']=>'add')); ?>'" /><?php } if($object->ui['reorder']!==null&&!in_array('reorder',$object->ui['disable_actions'])) { ?>
            <input type="button" value="Reorder" class="button" onclick="document.location='<?php echo pods_ui_var_update(array('action'.$object->ui['num']=>'reorder')); ?>'" /><?php } ?>
        </div>
<?php
            }
?>
...
?>
            <td class="post-title page-title column-title dragme">
                <strong style="margin-left:30px;"><?php echo $row['name']; ?></strong>
            </td>
<?php
                    continue;
                }
